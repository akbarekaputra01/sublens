...
// Tambahkan di bagian atas file app_httpd.cpp, setelah includes
...
static esp_err_t display_handler(httpd_req_t *req) {
  ...
  if (ret <= 0) {
    Serial.println("[ERROR] Failed to receive data from POST request");
    httpd_resp_send_500(req);
    return ESP_FAIL;
  }

  // Log untuk debugging
  Serial.print("OLED received data: '");
  Serial.print(buf);
  Serial.println("'");

  // Update OLED display
  display.clearDisplay();
  display.setTextSize(2);        // Text size yang sesuai untuk OLED 128x64
  display.setTextColor(SSD1306_WHITE);
  display.setCursor(40, 20);     // Posisi tengah layar
  display.println(buf);
  display.display();

  // Send response back to client
  const char* response = "OK - Data displayed on OLED";
  httpd_resp_send(req, response, strlen(response));
  
  Serial.println("[INFO] OLED display updated successfully");
  return ESP_OK;
}
...
void startCameraServer() {
  ...
  // Definisi display_uri (tapi belum register)
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    // Register semua handler SETELAH server berhasil dimulai
    ...
    // PENTING: Register display handler di sini, SETELAH server dimulai
    ...
    log_i("Display handler registered on /display endpoint");
  } else {
    log_e("Failed to start camera HTTP server");
  }
  ...
  if (httpd_start(&stream_httpd, &config) == ESP_OK) {
    ...
    log_e("Failed to start stream HTTP server");
  }
}
